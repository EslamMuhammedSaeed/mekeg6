@extends('foundation.master')
    <!--nav-->

 <!--nav-->
 <!--start main section-->
 @section('content')
 @include('foundation.inc.nav')
     
 
 <main class="mt-5">
     
    @if ($errors->any())
        <div class="container mt-5 text-left">
            @foreach ($errors->all() as $error)
                <div class="alert alert-danger">
                    {{ $error }}
                </div>
            @endforeach
        </div>
    @endif
     
     <form method="post" action="/test2" class="main-form was-validated" enctype="multipart/form-data">
        @csrf
         <div class="form-container container">
             <div class="row">
                 <div class="col-12 col-lg-4 pr-5 pt-5 ">
                     <div id="image_container" class="image-input position-relative">
                            <img id="trial" src="" class="img-fluid rounded-circle h-100">
                            <div class="svg-holder">
                                <svg  width="56" height="52" viewBox="0 0 56 52" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                    <rect width="56" height="52" fill="url(#pattern0)"/>
                                    <defs>
                                    <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                                    <use xlink:href="#image0" transform="translate(0 -0.039532) scale(0.00198807 0.002141)"/>
                                    </pattern>
                                    <image id="image0" width="503" height="504" xlink:href="data:image/png;base64,"/>
                                    </defs>
                                </svg>   
                            </div> 
                            <input id="input-image" class="is-valid" name="image" type="file" accept="image/png, image/gif, image/jpeg" required>
                            <div id="input-text" class="input-text ">أﺿﻒ ﺷﻌﺎر اﻟﻤﺆﺳﺴﺔ</div>
                     </div>
                 </div>
                 <div class="col-12  col-lg-8 ">
                   <div class="org-data">
                        <div class="mb-3 ">
                            {{-- <input type="text" name="image"> --}}
                            {{-- <input type="file" name="image"> --}}

                            <label  class="org-name">اﺳﻢ اﻟﻤﺆﺳﺴﺔ</label>
                            <input type="text" class="form-control is-valid" id="name" name="name" placeholder="" value="{{ old('name') }}"  required>
                        </div>
                        <div class="mb-3 ">
                            <label  class="org-name">وﺻﻒ ﻣﺨﺘﺼﺮ ﻋﻦ اﻟﻤﺆﺳﺴﺔ</label>
                            <textarea class="form-control is-valid" value="{{ old('description') }}" name="description" id="exampleFormControlTextarea1" required rows="3"></textarea>
                        </div>
                   </div>
                 </div>
             </div>
             <section class="essential-data">
                 <div class="heading">
                     <h3>اﻟﻤﻌﻠﻮﻣﺎت اﻟﺄﺳﺎﺳﻴﺔ</h3>
                 </div>
                 <div class="row">
                     <div class="col-md-6 col-lg-4">
                          <div class="mb-3 align-margin">
                                <label class="form-label">المحافظة</label>
                                <input type="text" name="govern" value="{{ old('govern') }}" class="form-control is-valid" required>
                          </div>
                          <div class="mb-3">
                                <label  class="form-label">القرية</label>
                                <input type="text" name="village" value="{{ old('village') }}" class="form-control is-valid">
                          </div>
                     </div>
                     <div class="col-md-6 col-lg-4">
                        <div class="mb-3 align-margin">
                              <label  class="form-label">المركز أو المدينة</label>
                              <input type="text" name="state" value="{{ old('state') }}" class="form-control is-valid" required>
                        </div>
                        <div class="mb-3">
                              <label  class="form-label">اﻟﺠﻬﺔ اﻟﺈدﺍﺭﻳﺔ اﻟﺘﺎﺑﻊ ﻟﻬﺎ اﻟﺠﻤﻌﻴﺔ</label>
                              <input type="text"  name="management" value="{{ old('management') }}" class="form-control is-valid" required>
                        </div>
                     </div>
                     <div class="col-md-6 col-lg-4">
                         <div class="local-unit">
                            <div class="mb-3">
                                <label  class="form-label">اﻟﻮﺣﺪة اﻟﻤﺤﻠﻴﺔ</label>
                                <input type="text" name="local" value="{{ old('local') }}" class="form-control is-valid" >
                            </div>
                            
                            
                         </div>
                     </div>
                 </div>

                 <div class="row mt-5 mb-5">
                     <div class="col-lg-6">
                        <div class="local-unit-date">
                            <h5>رﻗﻢ وﺗﺎرﻳﺦ اﻟﺈﺷﻬﺎر</h5>
                            <div class="row">
                                <div class="col-4">
                                    <label  class="form-label">رﻗﻢ</label>
                                    <input type="number" name="recordNumber" value="{{ old('recordNumber') }}" class="form-control is-valid w-100" name="number" required>
                                </div>
                                <div class="col-4">
                                    <label  class="form-label">السنة</label>
                                    <input type="number" class="form-control is-valid w-100" name="recordYear" value="{{ old('recordYear') }}" required>
                                </div>
                                <div class="col-4">
                                    <label  class="form-label">التاريخ</label>
                                    {{-- <input class="is-valid" required name="date w-100" type="text" onfocus="(this.type='date')" onfocusout="(this.type='text')" name="recordDate" type="text" onfocus="(this.type='date')" onfocusout="(this.type='text')" > --}}
                                    <input type="date" class="form-control is-valid" name="recordDate" value="{{ old('recordDate') }}" required>
                                </div>
                            </div>
                        </div>
                     </div>
                 </div>
                 
             </section>  
             <section class="personnel-info">
                 <div class="row">
                     <div class="col-md-6 col-lg-4">
                        <div class="heading">
                            <h3> مسؤول الاتصال الاول</h3>
                        </div>
                        <div class="mb-3 align-margin">
                            <label class="form-label">الاسم</label>
                            <input type="text" class="form-control is-valid"  name="firstPersonName" value="{{ old('firstPersonName') }}" required>
                        </div>
                        <div class="mb-3">
                                <label  class="form-label">اﻟﺼﻔﺔ اﻟﻮﻇﻴﻔﻴﺔ</label>
                                <input type="text" class="form-control is-valid" name="firstPersonRole" value="{{ old('firstPersonRole') }}" required>
                        </div>
                        <div class="mb-3">
                                <label  class="form-label">رقم الموبايل(واتساب)</label>
                                <input type="text" class="form-control is-valid" name="firstPersonPhone" value="{{ old('firstPersonPhone') }}" required>
                        </div>
                     </div>
                     <div class="col-md-6 col-lg-4">
                        <div class="heading">
                            <h3> مسؤول الاتصال اﻟﺜﺎﻧﻲ</h3>
                        </div>
                        <div class="mb-3 align-margin">
                            <label class="form-label">الاسم</label>
                            <input type="text" name="secondPersonName" value="{{ old('secondPersonName') }}" class="form-control background" >
                        </div>
                        <div class="mb-3">
                                <label  class="form-label">اﻟﺼﻔﺔ اﻟﻮﻇﻴﻔﻴﺔ</label>
                                <input type="text" name="secondPersonRole" value="{{ old('secondPersonRole') }}" class="form-control background">
                        </div>
                        <div class="mb-3">
                                <label  class="form-label">رقم الموبايل(واتساب)</label>
                                <input type="number" name="secondPersonPhone" value="{{ old('secondPersonPhone') }}" class="form-control background">
                        </div>
                     </div>
                     <div class="col-md-6 col-lg-4">
                            <div class="heading">
                                <h3>  مسؤول اﻟﺎﺗﺼﺎل اﻟﺜﺎﻟﺚ </h3>
                            </div>
                            <div class="mb-3 align-margin">
                                <label class="form-label">الاسم</label>
                                <input type="text" name="thirdPersonName" value="{{ old('thirdPersonName') }}" class="form-control" >
                            </div>
                            <div class="mb-3">
                                    <label  class="form-label">اﻟﺼﻔﺔ اﻟﻮﻇﻴﻔﻴﺔ</label>
                                    <input type="text" name="thirdPersonRole" value="{{ old('thirdPersonRole') }}" class="form-control" >
                            </div>
                            <div class="mb-3">
                                    <label  class="form-label">رقم الموبايل(واتساب)</label>
                                    <input type="text" name="thirdPersonPhone" value="{{ old('thirdPersonPhone') }}" class="form-control">
                            </div>
                     </div>
                 </div>
             </section>
             <section class="org-contact-info">
                <div class="heading">
                    <h3>ﻫﺎﺗﻒ اﻟﻤﺆﺳﺴﺔ</h3>
                </div>
                <div class="row">
                    <div class="col-md-6 col-lg-4">
                        <div class="mb-3 align-margin">
                            <label class="form-label">ارضي</label>
                            <input type="text" name="landPhone" value="{{ old('landPhone') }}" class="form-control is-valid">
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-4">
                        <div class="mb-3 align-margin">
                            <label class="form-label">موبايل</label>
                            <input type="text" name="mobilePhone" value="{{ old('mobilePhone') }}" class="form-control is-valid" required >
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-4">
                        <div class="mb-3 align-margin">
                            <label class="form-label">فاكس</label>
                            <input type="text" name="fax" value="{{ old('fax') }}" class="form-control ">
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-4">
                        <div class="mb-3 align-margin">
                            <label class="form-label">اﻟﻤﻮﻗﻊ اﻟﺈﻟﻜﺘﺮوﻧﻲ</label>
                            <input type="text" name="email" value="{{ old('email') }}" class="form-control background" >
                        </div>
                    </div>
                </div>
             </section>
             <section class="documents-copies">
                 <div class="row">
                     <div class="col-12 col-lg-6">
                        <div class="mb-3 align-margin">
                            <label class="form-label mb-3">ﺻﻮرة ﻣﻦ ﻗﺮاﺭ اﻟﺈﺷﻬﺎر</label>
                            <input id="input-1" name="recordDecision" type="file" class="form-control hidden is-valid" required>
                            <div   class="input-file right-svg position-relative py-3">
                                <svg  width="27" height="25" viewBox="0 0 27 25" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                    <rect width="27" height="25" fill="url(#pattern0)"/>
                                    <defs>
                                    <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                                    <use xlink:href="#image0" transform="translate(0 -0.0410736) scale(0.00198807 0.00214712)"/>
                                    </pattern>
                                    <image id="image0" width="503" height="504" xlink:href="data:image/png;base64,"/>
                                    </defs>
                                </svg>  
                                <div  id="filename-1" class="filename"></div>          
                            </div>
                        </div>
                     </div>
                     <div class="col-12 col-lg-6">
                        <div class="mb-3 align-margin" >
                            <label class="form-label mb-3">  <span class="custom">ﺧﻄﺎب اﻟﺘﻀﺎﻣﻦ اﻟﺈﺟﺘﻤﺎﻋﻲ</span>  (الجمعيه مشهرة وليس عليها اي مخالفات)</label>
                            <input id="input-2" type="file" name="socialLetter" class="hidden form-control is-valid"  required>
                            <div   class="input-file left-svg position-relative py-3">
                                <svg  width="27" height="25" viewBox="0 0 27 25" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                    <rect width="27" height="25" fill="url(#pattern0)"/>
                                    <defs>
                                    <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                                    <use xlink:href="#image0" transform="translate(0 -0.0410736) scale(0.00198807 0.00214712)"/>
                                    </pattern>
                                    <image id="image0" width="503" height="504" xlink:href="data:image/png;base64,"/>
                                    </defs>
                                </svg> 
                                <div id="filename-2" class="filename"></div>          
                            </div>
                        </div>
                     </div>
                 </div>
             </section>
             <div class="button-holder mb-5">
                <input type="submit" class="btn btn-success" value="حفظ">
             </div>
         </div>
     </form>
 </main>
 <!--end main section-->





    <!--start js files-->
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.16.0/umd/popper.min.js"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js"></script>
   
@endsection